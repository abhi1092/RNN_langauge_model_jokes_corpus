...
def get_word_rnn(prime_arg, n, sample_freq, pick=1, width=4):
    ...
    model = word_rnn.Model(saved_args, True)
    with tf.Session() as sess:
        tf.global_variables_initializer().run()
        saver = tf.train.Saver(tf.global_variables())
        ckpt = tf.train.get_checkpoint_state(save_dir)
        if ckpt and ckpt.model_checkpoint_path:
            saver.restore(sess, ckpt.model_checkpoint_path)
            ret = model.sample(sess, words, vocab, n, prime_arg, sample_freq, pick, width)
    ret = str(ret)
    ret = ret[1:]
    translator = str.maketrans(' ', ' ', string.punctuation[:13] + string.punctuation[14:])
    ret = ret.translate(translator)
...
def show_entry_fields():
    if model_type.get() == 'char_rnn':
        output.insert(INSERT, get_char_rnn_sample(save_dir=save_path.get(),
                                                  prime_arg=prime.get(),
...
if __name__ == '__main__':
    ...
    chosen_dataset.set(locations['l1'])
